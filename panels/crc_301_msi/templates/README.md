...
The golden template became the local default only after two distinct CRC301
cases passed generation, QA, privacy checks, and layout review. Keep the
standard aligned template available as the rollback path.
...
template. Runtime post-processing writes static TOC page numbers from the rendered
