...
- `1fbb2948410c75321530f93d6936498e07c9f3fd` 恢复六肺包 `warn_only`；建包脚本也
  显式设置同一默认，压制模式保留为 opt-in。`7922072fd46882839d344729c4664ccd1ecd3d92`
  只修正单测预期：没有压制时 QA 不生成 suppression 检查项，不是生成一个 PASS 项。
  两个提交已推送。66 + 211 项相关轻量测试与四模板/6 私有 token 交叉扫描通过。
- 隔离 `frozen_1fbb294` 三个小包已经真实网页 3/3 完成，九份 raw QA WARN、零 FAIL；
  A/B/C 页数：13 为 33/36/44，62 为 36/49/53，62+PD-L1 为 36/49/55。
  新 C13 只读核对四变异/三靶向、13/9/73 表格、38 项 588 parity 与四向泄漏均通过。
  无 PD-L1 588 也已 3/3（60/73/82 页，变异 7/8/9）；十二份原生空白门禁 PASS、
  16 项定向泄漏 PASS。588 三类检查器适配误报以同 Word 哈希独立回执解决，旧失败保留。
- B/C588 渲染前业务上下文对照旧生产 da8e62d：241 个旧键中仅
  `immune_hyperprogression_results` 因 #13 改变，另新增两项 CNV 待审字段；
  gene/drug 历史解析表完全一致。这不是最终 Word 或医学批准的替代证据。
- 四 draft 资格已绑定实际回执，三端仅禁用 CRC301/甲基化；48 项发布/范围测试通过、
  2 项平台跳过，scope gate PASS。606 页/35 张全书总览均已查看并核验哈希。
  生产尚未切换；仍须精确
  main CI、既有 CRC 历史合同/候选、备份及 immutable wrapper，不运行旧发布脚本。
- CI 34034116775 四个合成 draft gate PASS；完整后端 1087 passed / 2 skipped /
  1 failed，唯一失败为 329 旧扫描 PASS 预期。新断言验证原文可见 + WARN + 无压制，
  只改测试；最终发布提交仍必须重新跑完整 CI，不能把旧 CI 判为成功。
- 以下 0.11 的“医学 WARN 待授权/四 draft 禁用”是旧阶段快照，已由本节授权取代；
  原始生成/QA/失败回执保留。待决项统一在 `docs/analysis-decisions/lung-chemotherapy-pending-review.md`。

## 0.11 2026-09-06 小 Panel 派生输入（历史验收记录）

- 最新业务源码 `6462cd5bfc34beafb7d6ec7cd1dffc61451c02c6` 已提交/推送，远端 bundle
  SHA 已核，独立 `frozen_6462cd5` / 18099 / 23099 已完成九份真实网页报告：
  修复 B/C62 母版浮动表格整块跳页、统计说明孤页；四 draft 的动态标题用 Unicode
  项目符号，不再显示 Wingdings 字母 u。新增两项强制 QA，CRC/旧产品默认不改。
  279/33/10 项开发保护及四模板硬标识扫描通过。最新矩阵含自动空白页 9/9 PASS，
  C↔A / C↔B 定向泄漏 12/12 PASS；C13 四变异/三靶向、13/9/73 表格及 38 parity
  PASS；全部产品成员/HGVS/指南/逐药字段对照零失败，TSC1 原显示合同误报不再出现。
  原始 QA 九份 WARN、零 FAIL，严格 matrix=false；没有改写 WARN 或点击医学审核。
  13 / 62 / 62+PD-L1 页数分别 33/36/39、35/45/47、35/45/46，共 361 页。
  全书 22 张总览已下载/人工检查，59 项哈希一致：已确认 B/C62 标题孤页消除、符号正常；另发现少量
  页尾小标题/表头与次页正文分离，P2 排版待办未清零，详见审计 5.14 / finding 13。
  CI 34026187551 整条 SUCCESS，四 draft golden 和默认 qa-gate 均通过。正式生产仍
  da8e62d（18:27 指针/REVISION/cwd/公网健康已复核），main 295ebd2，未三源同步。
  医学 WARN / A IHC 来源 / 排版待办 / 主线合并 / 正式部署未完成，四 draft 禁用。
- 上一业务源码 `58834529ac5cf23b2043f7212a1a905967eacaa4` 已提交/推送：801f3b0
  先修复四 draft 缺少来源的样本类型从“组织”改为“未提供”，网页不再注入默认值；
  5883452 消除分节重新编号造成的自动空白页，原生 TOC QA 纳入这些此前被 PDF
  默认导出隐藏的页面；只让新 draft 连续编号，CRC 默认不变。另移除 62+PD-L1
  短指南指向说明前的多余分页。274 项开发、33 项 CRC、10 项旧渲染保护通过，
  四源模板基线和硬标识/6 token 交叉扫描通过。详见审计 5.12。
  37b77f7 真实网页九份均生成（3×3），但 A62+PD-L1 原始 QA FAIL，其余 WARN；
  原生核对证实九份各有一张 PDF 默认隐藏的自动空白页。C13 四变异/三靶向、
  13 基因/9 指南/73 PGx 及 38 项 588 parity 已在实际 Word 核对。旧失败全部保留。
  正在同步 `frozen_5883452`（18098/23098）进行新版完整矩阵；37 的三个任务终态后
  才停止旧验收实例。生产仍 da8e62d，四 draft 仍无生产资格，医学 WARN/A IHC/
  新版九例验收/主线合并/正式部署仍未完成。不要把生成完成当 QA PASS。
- 前一业务源码已提交/推送 `845aaef26eefeb9c659a9738223f4c85d2b69085`：四 draft
  主变异表取消继承正文缩进、重复两行表头、事件行不拆页；三个小包空药物栏目显式
  提示暂无条目/待审并消除占位分页，未删真实药物行或修改医学规则。213 + 33 项回归、
  四模板源基线和 6 token 交叉扫描通过；新版 Linux/Web/CI 待完成。
  f7aabde 的完整 CI 34018704771 已 SUCCESS；真实 C13/A13 为 40/35 页，C13
  4 变异/3 靶向、13 基因/9 指南/73 PGx，A13 为 1 变异/9 指南/71 PGx；分表核对与
  限定 token 双病例检查 PASS，但原始 QA 均 WARN。人工看 C13 第 6—8 页发现本轮
  新修问题，不能借机器空白门禁宣称专业版式通过。旧 Word/PNG/失败回执全部保留。
  f7 Web 首轮 13 批量 0/3：私有入口缺 main 保护导致 spawn 重复绑端口，未生成稿；
  测试入口已修复并验证导入无副作用，两个旧测试实例按 PID/cwd 精确停止，生产未动。
  正在准备 `frozen_845aaef`（18095/23095）。真实九例、医学 WARN 裁决、A 的 IHC
  来源、主线合并及正式部署未完成；四 draft 继续无生产资格。详见审计 5.9—5.10。
- 前一业务源码已提交/推送 `0b85b93bdc3c7d531637c5d4bb86012d28c3328f`，新增持久标题
  防止原生引擎合并指南/多药 PGx；合成 golden 改两种药并检查最终表格边界。
  208 项开发 + 33 项 CRC 保护 PASS，四模板硬标识/跨母版 token 零命中；本轮 Linux
  回放与 CI 尚待结果，不能宣布部署。1244f3d 同时修复 B 族旧目录装饰及 FAQ 孤页。
  18514ad 的真实 C13 已为 39 页、4 变异/3 靶向、13 基因/9 指南/73 PGx，空白门禁 PASS，
  原始 QA WARN；A62 35 页/WARN。行均保留，但相邻表被 LO 合并；新比对器正确读逻辑格，
  38 项 588 parity 只规范 None→空文本后零差异，合并仍阻断。原始 FAIL 全部保留。
  CI 34017234879 四 draft 合成 gate 全 PASS，但默认 qa-gate 未完成，且不覆盖新修复。
  私有 Web 18514ad（18093/23093）已登录，C62 Excel 默认非 IHC、实际 TPS 5/CPS 6 后
  自动选 62+PD-L1；未提交生成/批量。真实九例、双病例泄漏、医学 WARN 定义、A 的 IHC
  来源、主线合并与部署仍未完成；四 draft 仍无生产资格。详见审计 5.7—5.8。
- 前一修复源码 `e7d343a8d38e6d3babcb8c96d5eb096b2d5aa257`：人工发现 C 族有效页码
  目录混入长篇文献及局部参考文献标题；限定标题范围并补文献条目/URL 混入 QA。
  只重建 C 族模板，200 + 33 项回归通过，待 Linux 重放。CI 34016832113
  （fb1e725，业务源码 94b478e）四 draft 合成完整 gate 全 PASS，但旧 C 族目录
  不等于人工专业性通过。两个 bundle 已核完整远端 SHA，bdd7eaa 对象已导入。
  B/C IHC 图片和 YAML 远端 SHA 已核对；A 来源缺失不填。准备最新隔离回放；
  真实九例/Web UAT/医学 WARN 定义/部署仍未完成，四 draft 继续禁用。
- 前一修复源码 `94b478e4cd8efa9571a30fdc352efdd034886089`：588 旧目录藏在浮动文本框，
  原生目录又误收录病例单元格/空段落；现已移除旧缓存、限定真实标题并移动分节边界，
  清理标题前冗余空段落。QA 增加浮动旧目录残留检查，并正确识别取消编号 numId=0。
  只更新 588 模板/基线，180 + 17 + 33 项轻量保护通过；新 Linux 渲染尚待验证。
  CI 34015953334（83b22b0，业务源码 fb72cc3）13/62/62+PD-L1 完整合成 gate PASS
  （31/27/29 页），588 FAIL（53 页，历史目录和第 9/27 页低内容），失败产物保留。
  14:30 SSH 短连接恢复，但远端 GitHub fetch 40 秒超时；正在传 bundle，完整 SHA
  未核验前不导入。真实九例、Web UAT、医学 WARN 定义和部署仍未完成，不开放 draft。
- 前一修复源码 `fb72cc33490d21b4f0ef3a0ba8184534f9bb4580`：移除 62+PD-L1 历史
  “2 变异/0 靶向”摘要，增加每处统计一致性 QA；基本信息变量显式黑字；目录配置
  native、最终原生刷新、缺域失败，并修复外层 QA 加载器漏传章节别名。
  191 项轻量 + 33 项 CRC/目录/QA 保护通过；新四模板 SHA 见审计 5.4/第 2 节。
  8c8aeb5 的合成 13/62 原始 QA PASS（31/28 页），但 62+PD-L1/588 FAIL
  （空目录/空白页）；人工看图才发现旧统计和白字。当前修复尚待新 Linux CI。
  网络阻塞持续；真实案例、网页批量、双病例泄漏和部署都未完成，不能开放草稿。
- 前一修复源码 `fa17b1ab2413cb1c65a5c5268176f49961051163`：修复参考文献清理误删
  后续补充章节；62+PD-L1/588 无 PD-L1 改原生可刷新目录；四包 CI 启用整本视觉 QA。
  当前轻量 176 passed，6 个私有 token × 4 模板硬命中 0。真实生成仍停留在
  frozen_6749292；服务器 GitHub/SSH 反复超时，部分 Git bundle 未验完整 SHA 不可导入。
  4387fe8 四个新合成 golden gate 均 FAIL，产物已下载；此次修复需新 CI/服务器回放。
  B/C IHC 文件已传，远端图片 SHA 尚待核验；隔离 Web 服务尚未启动，未部署。
- 前一修复源码 `411faf12cd5f81ed327e8117e02a27836b5aa800`：补齐逐药 PGx/剂量、
  CNV 待复核显示、原生 SDT 目录检查与短基因/QC 附录分页；新增最终 Word 指南/
  基因列表/逐药六字段比较和四 draft 合成 golden runner。开发 113 + 31 项通过，
  6 个私有源 token × 4 包交叉扫描零硬命中；尚待该身份整本重放，未部署。
- 6749292 首轮实稿：C13 四变异/三靶向已由 Word 独立读回确认，但 30 页中第
  26/27 页空白/低内容，原始 QA FAIL；A62 五变异、27 页，因漏 CNV 待复核提示
  QA FAIL。两例 TMB/MSI/化疗上下文与 588 无差异。旧 FAIL 和 PNG 保留，不能由
  简易空白检测 PASS 或后续开发修复覆盖；完整记录见对应 Codex audit 5.2。
- 已向用户非阻断询问：工程 PASS 是否允许保留医学待审 WARN。尚无答复，原始
  QA 与验收脚本仍严格要求 PASS，未实施豁免。全部新产品仍 draft / production=false。
