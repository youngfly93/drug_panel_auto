...
> 证据来源：`.work/uat_evidence/` 下脱敏 JSON（含新增单例渲染 receipt）+ 冻结源码。未读取任何 agent 运行态/会话/缓存/患者文件。
> 沙箱说明：本机沙箱拦截了 `pytest`/`python3`/`shasum` 执行，回归测试与 receipt 哈希为**静态核验**（读代码+读证据），未实跑/未本地复算 SHA；已在下文如实标注。

## 0. 已锁口径（审核合同）
1. 四级分开裁决：**工程候选** ≠ **医学/UAT** ≠ **promotion** ≠ **生产部署**；任一上层不因下层工程 PASS 而自动成立。
2. 证据分层：exact-SHA 执行证据（语义扫描、发布门禁 replay、单例渲染 receipt）与"数据可得性"证据（备份历史、真实清单）不可混用；后者用于"有没有料"，不能当 exact-SHA 执行结论。
...
| report-semantic-gaps-11 | P2 | 本 SHA(b7fa69a) 已完成全量 Word 渲染 + 人工视觉复核。 | 新增 exact-SHA 单例渲染 receipt `.work/uat_evidence/public_crc358_single_render_prodmode_b7fa69a.json`（run_id `crc358-single-linux-prodmode-b7fa69a-20260722`，`candidate_revision=b7fa69a…60ef8101`，`environment=iyun129-isolated-linux-candidate`，panel `crc_358_msi`；receipt SHA256 `ab32d20ed2af474432001ad47026068c205ffd099b04d6fbf254694757d6ab9e`，因沙箱拦截 `shasum`/`python3` 未本地复算、按提供值记录）。该 receipt 证明：**一份** b7fa69a 的 iyun129 隔离 Linux 报告成功生成（case UAT358-01，`generation_success=true`，`rendered_page_count=96`），`visual_render_status=PASS`；子项 `toc_page_numbers`/`blank_page_detection`/`drug_analysis_consistency`/`gene_domain_coverage`/`msi_status_text`/`tmb_status_text`/`field_provenance`/`docx_openable`/`template_contract`/`visual_render` 全 PASS；整体 `qa_status=WARN`，仅 `drug_analysis_contract_coverage=WARN`（issue_codes `DRUG_ANALYSIS_CONTRACT_COVERAGE`、`PIPELINE_WARN`）。**但** claim 需要"全量 Word 渲染 + 人工视觉复核**完成**"，而 receipt 明确为 `cases_expected=1`、`uat_gate_status=SAMPLE_RENDER_ONLY`、`machine_passes=0`、`human_visual_review_complete=false`、case `human_visual_conclusion=PENDING`——即单例样本渲染、非全量，人工视觉复核仍 PENDING；`knowledge_release_gate…` `uat.reviewed_reports=0` 亦未变。故"已完成全量渲染+人工视觉复核"仍被证伪。 | REFUTED |
| report-semantic-gaps-12 | P1 | 生产已切到本 SHA 并完成现网验收。 | 无 b7fa69a 部署证据；HANDOFF 明确"候选尚未提交、未部署，生产仍须按实时四项证据确认"；审核禁连生产。新增单例渲染 receipt 仅为 iyun129 隔离 Linux 候选环境（`environment=iyun129-isolated-linux-candidate`）的样本渲染，**非生产、非 post-deploy 现网验收**。无任何证据表明生产已切到本 SHA，更无现网验收；候选处于冻结、未部署态，claim 不成立。 | REFUTED |
| report-semantic-gaps-13 | P1 | 候选现在可以 promote。 | 综合 04/05/06/07/08/11/12 + 门禁 `clinical_release_readiness=BLOCKED`。医学/UAT 阻断（语义 PASS 20%、3 例真实漏药、22 个未契约化变异、CRC301 0 例、UAT reviewed=0）；单例样本渲染虽已证隔离 Linux 可渲染，但十例全量与人工视觉复核未完（report-semantic-gaps-11）。promotion/生产前置未满足；候选**现在不可 promote**。 | REFUTED |
...
- **B-02（P1，UAT）**：exact-SHA 语义 PASS 率 20%（2/10），远低于 90%；报告组真实报告 UAT reviewed=0。发布 UAT 阈值未达。单例渲染 receipt 证明隔离 Linux 单份可成功渲染（96 页、视觉 PASS），但 `uat_gate_status=SAMPLE_RENDER_ONLY`、`human_visual_review_complete=false`——十例全量与人工视觉复核仍未完。
- **B-03（P1，覆盖）**：CRC301 真实历史输入 = 0，无法起 CRC301 病例 UAT。
- **B-04（P2，治理）**：22 个 legacy 未契约化变异、8/10 例 MIGRATION_PENDING；契约迁移未完。（与单例 receipt 的 `drug_analysis_contract_coverage=WARN` 同向：该单例契约覆盖亦仅 WARN 未 PASS。）
...
- exact-SHA 隔离 Linux **单例**渲染已证可行：b7fa69a 一份 96 页报告成功生成、视觉 PASS、TOC/空白页/药物一致性/结构域/MSI/TMB/字段溯源子项全 PASS（report-semantic-gaps-11 evidence 内）；仅为样本，不代表全量/人工复核完成。

## 3.5 方法保真表（method-fidelity · 共享审核合同要求）

> 裁决方法本身是否忠实：只用 FAITHFUL / HONEST_BOUNDARY / UNDISCLOSED_DOWNGRADE / FALSE_REASON。
> 本表仅评估"我审核所用方法与被强制方法是否一致"，不改动上文 13 条发现的任何裁决/身份/结论。沙箱说明（见 §4 caveats）继续有效：`pytest`/`python3`/`shasum` 部分执行被本机沙箱拦截，我未做任何远端 blob 校验、未本地复算 receipt SHA、未连生产。

| id | mandated 方法 | 实际 method_status | verdict | evidence |
|---|---|---|---|---|
| report-semantic-gaps-M01 | candidate、隔离 Linux 扫描、审核三者须绑定同一 Git 身份。 | 我用本地 `git rev-parse HEAD` 确认 checkout 精确等于 b7fa69a（report-semantic-gaps-01），exact-SHA 执行证据文件名/内容为 `…_b7fa69a`（语义扫描、发布门禁、单例渲染 receipt `candidate_revision=b7fa69a…60ef8101`）；但"数据可得性"证据（`public_real_crc358_inventory.json`、`public_backup_panel_history.json`）标注 `candidate_revision=f032ff3`，与冻结审核 SHA 不同。我**未执行任何远端 blob / 远端 candidate 二进制比对**，也**未本地复算 receipt SHA**（沙箱拦截），只在本地 checkout 与 exact-SHA 证据之间绑定身份，并已把 f032ff3 明确降级为"仅可得性用"（report-semantic-gaps-08）。 | HONEST_BOUNDARY | `git rev-parse HEAD`=b7fa69a…60ef8101；`public_crc358_semantic_scan_b7fa69a.json`、`knowledge_release_gate_b7fa69a.json`、`public_crc358_single_render_prodmode_b7fa69a.json`（三者 exact-SHA）；`public_real_crc358_inventory.json`/`public_backup_panel_history.json` `candidate_revision=f032ff3`（可得性，非本 SHA 执行）。未做远端 blob 校验、未本地复算 receipt SHA。 |
| report-semantic-gaps-M02 | 真实病例 UAT 公开证据须保持脱敏。 | 直接读 `privacy` 字段与可见键：清单 `privacy="No patient name, sample identifier, original filename, or source path is included."`，可见键仅 `case_id`+`sha256`+`sheet_count`+`required_fields_complete_in_source`；备份历史 `privacy="Only aggregate panel counts are included."`；新增单例 receipt `privacy="Only aliases, hashes, aggregate counts, and QA status are public."`，可见键仅别名 `case_id=UAT358-01`+各 sha256+聚合计数+QA 状态。无姓名/样本号/原文件名/路径/患者可识别字段泄露；与 report-semantic-gaps-02"提交无 PII"一致。此项可从字段与可见键直接核验，方法按强制执行。 | FAITHFUL | `public_real_crc358_inventory.json` `privacy` + 可见键；`public_backup_panel_history.json` `privacy=aggregate counts only`；`public_crc358_single_render_prodmode_b7fa69a.json` `privacy=aliases/hashes/counts/QA only`。 |
...
| report-semantic-gaps-M04 | 生产结论须有 exact-SHA Linux 全量渲染 + post-deploy 现网证据。 | 证据部分更新：现有 exact-SHA 隔离 Linux **单例**渲染 receipt（`public_crc358_single_render_prodmode_b7fa69a.json`：b7fa69a 一份 96 页报告成功生成、视觉 PASS、多项子检查 PASS、整体 QA 仅 WARN），证明隔离 Linux 单份渲染可行；**但**它是 `SAMPLE_RENDER_ONLY`、`cases_expected=1`、`human_visual_review_complete=false`，**既非全量渲染、也非 post-deploy 现网证据**（`environment=iyun129-isolated-linux-candidate`、非生产）。`uat.reviewed_reports=0` 未变。审核禁连生产，我未连生产、未实测现网、未本地复算 receipt SHA。因此我把生产/promotion 结论严格界定为 NOT DEPLOYED / NOT READY（report-semantic-gaps-11/-12/-13 REFUTED），基于"样本渲染已证、全量与人工复核未完、无现网"的诚实边界作否定，未越界断言"已全量渲染/已现网验收/已部署"。 | HONEST_BOUNDARY | 有 exact-SHA 单例渲染 receipt（`public_crc358_single_render_prodmode_b7fa69a.json`，`uat_gate_status=SAMPLE_RENDER_ONLY`、`human_visual_review_complete=false`）；无全量渲染、无 post-deploy 现网证据；`knowledge_release_gate_b7fa69a.json` `uat.reviewed_reports=0`；report-semantic-gaps-11/-12/-13。审核禁连生产、未本地复算 SHA。 |

## 4. 分级裁决（结论）
- **工程候选（Engineering）：PASS。** 冻结干净、边界修复根治+回归、门禁工程状态 PASS、二审 receipt hash 有效；exact-SHA 隔离 Linux 单例渲染已证可行（96 页、视觉 PASS）。
- **医学 / UAT：BLOCKED（FAIL）。** exact-SHA 语义 PASS 20%、3 例真实漏药（B-01）、22 个未契约化变异、CRC301 0 例、报告组 UAT reviewed=0。不得表述为医学审完。
- **Promotion：NOT READY（REFUTED，report-semantic-gaps-13）。** 医学/UAT 前置未满足；同-SHA CI 单独不足（report-semantic-gaps-10）；隔离 Linux **单例样本**渲染已证可行（b7fa69a 一份 96 页、视觉 PASS），但**十例全量渲染与人工视觉复核未完**（`uat_gate_status=SAMPLE_RENDER_ONLY`、`human_visual_review_complete=false`；report-semantic-gaps-11）。
- **生产部署（Production）：NOT DEPLOYED（REFUTED，report-semantic-gaps-12）。** 无本 SHA 部署与现网验收证据；单例 receipt 属 iyun129 隔离候选环境、非生产；切换须以 `current_release`/完整 `REVISION`/进程 cwd/健康检查四项实时证据为准。

### 沙箱说明（caveats）
- 本机沙箱拦截 `pytest`/`python3`/`shasum` 执行，report-semantic-gaps-03 的回归为**静态核验**（读代码+读证据）未实跑；receipt SHA256 `ab32d20…57d6ab9e` 按提供值记录、未本地复算。
- 审核禁连生产：现有 exact-SHA 隔离 Linux **单例**渲染 receipt 已证 b7fa69a 单份可成功渲染（96 页、视觉 PASS），但为 `SAMPLE_RENDER_ONLY`、人工视觉复核 PENDING、非全量、非现网；report-semantic-gaps-11/-12 的全量渲染/生产结论仍基于"全量+人工复核+现网"缺证，非现网实测。
...
- 方法保真表（§3.5）中 report-semantic-gaps-M01 未做远端 blob 校验/未本地复算 receipt SHA、report-semantic-gaps-M03 的 current-upload 实时 rescan 不在本审证据范围内、report-semantic-gaps-M04 仅有单例样本渲染 receipt（非全量/非现网）——均已如实标注为 HONEST_BOUNDARY。
