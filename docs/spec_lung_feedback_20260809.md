...
- 冻结工程提交：`cd3069e47f214213db367a3db0b8231e1c9332b2`
...
| 报告组在审核前看不到 Word | 下载动作错误依赖“已审核”，形成必须先审核、后查看报告的循环门禁 | 肺癌生成完成后即可下载；未审核时按钮及下载文件名均标识“草稿”。审核状态继续记录正式复核/交付过程，但不阻止报告组取得待审 Word。 |
...
- 未审核肺癌下载文件名回归：329/588 均使用“草稿”，reviewed/delivered
  恢复配置的正式版本标签；QA FAIL 下载约束保持不变，定向 2/2 通过。
...
- 独立审计对草稿工作流提交 `6af2a356...` 的工程结论为 PASS、P0/P1=`0/0`；
  ...
  均已在 `e791778...` 中对齐；未审核下载文件名又在冻结工程提交
  `cd3069e...` 中改为“草稿”，未新增运行时医学事件。
