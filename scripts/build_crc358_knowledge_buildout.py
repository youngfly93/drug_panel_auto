...
def dedupe_candidates(candidates: Iterable[Candidate]) -> list[Candidate]:
    ...
    for cand in candidates:
        ...
        key = (cand.product_family, key_gene, c_hgvs, p_hgvs, cand.content_type, cand.drug_name, text_key)
...
def scan_reports(corpus: Path, overlay: Path, civic: Path) -> tuple[list[dict[str, object]], list[Candidate], list[Candidate]]:
    ...
    for path in sorted(corpus.iterdir(), key=lambda p: p.name):
        ...
        try:
            ...
            if product_family == "crc_358_msi" and is_final:
...
def main() -> int:
    ...
    inventory, raw, deduped = scan_reports(args.corpus, args.overlay, args.civic_pilot)
