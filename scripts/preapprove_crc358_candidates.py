...
def should_preapprove(row: dict[str, str]) -> tuple[bool, str]:
    ...
    if content_type in {"mutation_analysis", "drug_relation", "drug_clinical"} and not (
        row.get("c_hgvs") or row.get("p_hgvs")
    ):
        return False, "非基因简介但缺位点信息"
    if content_type == "mutation_analysis" and row.get("confidence") != "高":
        return False, "位点解析需高置信重复来源"
    if (
        row.get("product_family")
        and row.get("product_family") != "crc_358_msi"
        and content_type != "gene_intro"
    ):
        return False, "跨panel候选仅允许基因简介自动预审"
    if content_type in {"drug_relation", "drug_clinical"}:
        if row.get("confidence") != "高" or as_int(row.get("source_count", "")) < 3:
            return False, "药物解析需高置信且至少3份终版重复"
        if not row.get("drug_name") or row.get("drug_name") == "--":
            return False, "药物解析缺药名，需人工整理后再入库"
        if row.get("drug_type") not in {"benefit", "caution"}:
            return False, "药物类型不明确，需人工整理后再入库"
