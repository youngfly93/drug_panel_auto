#!/usr/bin/env python3
# 步骤: 72 肺癌329受控试运行合成边界报告验证
# 上游: panels/lung_329_pdl1/、config/、肺329模板
# 输出: .work/lung329_pilot_boundary_suite/validation.json 与7份合成报告
# 种子: 20260725（固定合成病例，不含真实患者信息）
"""Generate seven deterministic, de-identified lung329 pilot boundary reports."""
...
SCENARIOS: tuple[dict[str, Any], ...] = (
    ...
    {
        "id": "SYN-L329-04-PDL1-HIGH-LOWER",
        "tps": 50,
        "cps": 52,
        "result": "阳性（高表达）",
        "tmb": 10,
    ...
    {
        "id": "SYN-L329-05-PDL1-HIGH-UPPER",
        "tps": 100,
        "cps": 100,
        "result": "阳性（高表达）",
        "tmb": 20,
...
def run(output_dir: Path, *, require_visual: bool, dpi: int) -> dict[str, Any]:
    ...
    for scenario in SCENARIOS:
        ...
        if not output_path.is_file():
            ...
            if f"SYNTHETIC-IHC-{case_id}" not in visible:
                failures.append("missing_case_specific_source_record")
            if "本病例未提供可追溯的PD-L1免疫组化图像" not in visible:
                failures.append("missing_image_disposition_notice")
    ...
    payload = {
        ...
        "suite_id": "lung329_controlled_pilot_synthetic_boundary_v1",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=(ROOT / ".work" / "lung329_pilot_boundary_suite_20260725"),
    )
